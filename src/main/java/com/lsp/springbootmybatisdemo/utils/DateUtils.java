...
public class DateUtils {
	...
	public static Date convertStringToMoreDate(String str) {
		...
		if (!StringUtils.isEmpty(str)) {
			try {
				...
				} else if (str.length() == monthLength) {
	 ...
	 * @return
	 */
	/*public static QueryFilter buildTimeFilter(QueryFilter queryFilter, int day, String property) {
		Calendar calendar = Calendar.getInstance();
		Date date = new Date();
		calendar.setTime(date);
		Date sDate = null;
		...
		switch (day) {
		case 1:     //查询当前的
			sDate = DateUtils.startOfDay(date); //当天开始时间
			eDate = DateUtils.endOfDay(date); //当天结束时间
			break;
		case 3:     //查询3天内的
			calendar.add(Calendar.DATE, -3);
			Date bdate = calendar.getTime();
			sDate = DateUtils.startOfDay(bdate); //当天开始时间
			eDate = DateUtils.endOfDay(date); //当天结束时间
			break;
		case 7:     //查询本周内的
			...
			calendar.add(Calendar.DATE, 2 - dayofweek); //获取起始时间
			Date qsdate = calendar.getTime(); //获取本周星期一日期
			
			calendar.add(Calendar.DAY_OF_WEEK, 6);//获取本周星期六
			Date jsdate = calendar.getTime(); //获取本周星期一日期
			sDate = DateUtils.startOfDay(qsdate); //当天开始时间
			eDate = DateUtils.endOfDay(jsdate); //当天结束时间
			break;
		case 30:     //查询本月内的
			calendar.set(getNowYear(date), getNowMonth(date) - 1, 1);
			Date yqsdate = calendar.getTime(); //获取本月起始日期
			
			int days = calendar.getActualMaximum(5);
			calendar.set(getNowYear(date), getNowMonth(date) - 1, days);
			Date yjsdate = calendar.getTime(); //获取本月起始日期
			
			sDate = DateUtils.startOfDay(yqsdate); //当天开始时间
			eDate = DateUtils.endOfDay(yjsdate); //当天结束时间
			break;
		default:
			break;
		}
		queryFilter.addFilter(property, sDate, MatchType.GE);
		queryFilter.addFilter(property, eDate, MatchType.LE);
		return queryFilter;
	}
	*/
	public static void main(String[] args) throws Exception {
		Date nowTime = new Date();
		String endTime = DateUtils.convertDateToString("yyyy-MM-dd'T'HH:mm:ss+08:00", nowTime);
