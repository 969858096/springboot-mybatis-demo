...
import com.lsp.springbootmybatisdemo.test.设计模式.工厂模式.抽象工厂.chairEntity.ChairA;
import com.lsp.springbootmybatisdemo.test.设计模式.工厂模式.抽象工厂.chairEntity.ChairB;
import com.lsp.springbootmybatisdemo.test.设计模式.工厂模式.抽象工厂.engineEntity.EngineA;
import com.lsp.springbootmybatisdemo.test.设计模式.工厂模式.抽象工厂.engineEntity.EngineB;
    ...
    public Engine makeEngine(String name) {
        ...
        if (StringUtils.isNotBlank(name)) {
            if (name.equalsIgnoreCase("engineA")) {
                engine = new EngineA();
            } else if (name.equalsIgnoreCase("engineB")) {
                engine = new EngineB();
            } else {
                log.info("不存在");
            }
    ...
    public Chair makeChair(String name) {
        ...
        if (StringUtils.isNotBlank(name)) {
            if (name.equalsIgnoreCase("chairA")) {
                chair = new ChairA();
            } else if (name.equalsIgnoreCase("chairB")) {
                chair = new ChairB();
            } else {
                log.info("不存在");
            }
